...
| 2 | **services** (modules + use cases) | DONE | Combined section — was split into `orbital` (modules) and `tailored` (use cases) which read as two parallel explorers and confused the modules-vs-use-cases relationship. Now one section: zone 1 is the orbital (the toolkit), zone 2 is a clean use-case grid (how the toolkit combines). Module chips on each card scroll back up and open the relevant module. |
| 3 | **framework** (5-step explainer) | DONE | Five tiles (Learn → Think → Build → Execute → Deliver) with hover-peek + click-to-lock to expose a stage panel showing the activity for each step. |
...
**Framework needs this** — ~54k chars, over the 50k limit because of the five SVG illustrations on the stage panel.

**Services needs this too** — ~82k chars (orbital's 50k + the MODULES data with 6 inline SVG previews + the new use-case grid). Move the contents of the trailing `<script>...</script>` block into Project Settings → Custom Code → Before `</body>` (just the JS body, no `<script>` tags), and paste only the `<style>` + `<section>` markup into the page Embed component.

Intro and the smaller sections are well under the limit.
...
| Apr 2026 | Orbital + tailored merged into a single `02-dmc01-services` embed (on branch `redesign/tailored-b-merger`). Two zones in one file: `01 · The toolkit` (the orbital — kept) and `02 · How they combine` (a clean 3×2 grid of use-case cards, each with the 2–3 module chips it pulls in — chips scroll-link back up to the orbital and open the relevant module). The old tailored's board panel, "what we're experts in" chips, constellation, and double paragraph were dropped. | The previous two-section structure (orbital = modules, tailored = use cases) had matching visual weight + matching click-to-explore patterns, which read as two parallel explorers rather than as toolkit + combinations. Numbered zone eyebrows + radically simpler use-case cards make the relationship explicit. Cross-section dmc01:openModule event survived for compatibility (still works from anywhere); the in-section chip handler is the new primary path. |
